...
def process_set(sets):
    if isinstance(sets[0], (set, list)):
        return [set(s) for s in sets]

    comms = []
    labels = set(sets)  # ✅ 改回 set()
    for label in labels:
        comm = set(np.where(sets == label)[0])
        comms.append(comm)
    return comms
...
def cal_F_score_helper(found_sets, GT_sets):
    d1, d2 = {}, {}
    for i, gt in enumerate(GT_sets):
        f_max = 0
        for j, f in enumerate(found_sets):
            common_elements = gt & f
            temp = F_score_helper(gt, f, common_elements) if common_elements else 0
            if j not in d1 or temp > d1[j]:
                d1[j] = temp
            f_max = max(f_max, temp)
        d2[i] = f_max
    return d1, d2
...
def cal_F_score(found_sets, GT_sets, verbose=False):
    ...
    vals1 = sum(d1.values()) / len(d1)
    vals2 = sum(d2.values()) / len(d2)
    f_score = (vals1 + vals2) / 2

    return round(f_score, 6), round(vals1, 6), round(vals2, 6)
...
def matched(true,pred):
    ...
    g = nx.from_numpy_array(shifted_mat)
...
#print ("f1:",score[0])
def adjusted_rand_score(pred,true):
    ari_score = Metrics.adjusted_rand_score(pred, true)
    return ari_score