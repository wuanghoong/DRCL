...
def load_graph_data(root_path=".", dataset_name="dblp", show_details=False):
    ...
    :param dataset_name: the name of the datasets
    :param show_details: if show the details of datasets
    - datasets name
    - features' shape
    - labels' shape
    - adj shape
    ...
    if not os.path.exists(dataset_path):
        ...
        logging.info("Downloading " + dataset_name + " datasets from: " + url)
    else:
        logging.info("Loading " + dataset_name + " datasets from local")
    ...
    if show_details:
        print("++++++++++++++++++++++++++++++")
        print("---details of graph datasets---")
        print("++++++++++++++++++++++++++++++")
        print("datasets name:   ", dataset_name)
...
def load_data(dataset_path, dataset_name,
    ...
    :param dataset_path: The store path of datasets.
    :param dataset_name: The name of datasets.
    ...
    """
    # If the datasets is not graph datasets, use construct_graph to get KNN graph.
    ...
    if feature_type == "tensor":
        feature = numpy_to_torch(feature.astype(np.float32))
    if adj_type == "tensor":
        adj = numpy_to_torch(adj)
    if label_type == "tensor":
        label = numpy_to_torch(label)
    data = Data(feature, label, adj)
    return data
